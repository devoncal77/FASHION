...
def cumulative_metric(nodes, edges):
	...
	while U != []:
		...
		if n in nodes[2]:
			P = find_P(n, edges)
			prob_table[n] = 1 - (1 - evalProb(P, negate_boo(node_boo), nodes, edges))
			...
			P = find_P(n, edges)
			prob_table[n] = Gm[n] * evalProb(P, negate_boo(node_boo), nodes, edges)
			chi_n(n, nodes, BRANCH_nodes, P, edges)
			delta_n(n, nodes, BRANCH_nodes, P, edges)
		U.remove(n)
	return	
...
def evalCondProb(D, N, node_boo, nodes, edges):
	...
	if len(N) > 1:
		...
		cond_prob = 1 - evalCondProb(D, [N[0]], negate_boo(node_boo), nodes, edges)
		return cond_prob
	else:
		N = N[0]
		if D != []:
		...
		P = find_P(N, edges)
		if N in nodes[2]:
			return 1 - evalCondProb(find_D_set(P, chi_n_table), P, negate_boo(node_boo), nodes, edges)
		elif N in nodes[1]:
